// Package rtree implements point-in-polygon lookup with an in-memory R-tree.
//
// The provider builds its index from the embedded `earth.geojson` asset. Queries
// first use the R-tree to find candidate geometries by bounding box, then run an
// exact point-in-polygon check before returning the dataset's country code and
// continent name.
//
// The embedded GeoJSON asset is generated by `scripts/update-lookup-assets`.
// That script filters the source data to country features the provider can
// index and trims properties to the country code (`iso_a2` or `iso_a2_eh`) and
// `continent` values used by the lookup path.
//
// Indexed features are expected to use Polygon or MultiPolygon geometry. The
// lookup path assumes that invariant when checking candidate nodes.
package rtree
